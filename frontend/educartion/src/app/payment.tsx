...
import type { DeliveryAddress, PaymentDetails } from "@/lib/payment-contract";
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    const loadCheckout = async () => {
      const cartData = await paymentController.handleLoadCheckoutCart();
      setCart(cartData);
      setLoaded(true);
    };
    loadCheckout();
  ...
  const confirmOrder = async () => {
    ...
    const deliveryAddress: DeliveryAddress = { name, address, city, postal };
    const paymentDetails: PaymentDetails = { cardName, cardNumber, expiry, cvc };

    const result = await paymentController.handleSubmitCheckout({
      deliveryAddress,
      paymentDetails,
    });
  ...
  return (
    <main style={{ width: "100%", padding: 24, display: "grid", justifyItems: "center" }}>
      <div style={{ width: "100%", maxWidth: 900, display: "grid", gap: 20 }}>
        ...
        {error && (
          <div style={{ padding: 12, borderRadius: 8, background: "#fee", border: "1px solid #f88", color: "#c33" }}>
            {error}
          </div>
        )}
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    style={{ padding: 10, borderRadius: 8, border: "1px solid #d1d5db", flex: 1 }}
                  />
                  <input
                    placeholder="Postal code"
                    value={postal}
                    onChange={(e) => setPostal(e.target.value)}
                    style={{ padding: 10, borderRadius: 8, border: "1px solid #d1d5db", width: 120 }}
                  />
                </div>
              </div>
            </div>
          ...
          <aside style={{ display: "grid", gap: 12, alignSelf: "start" }}>
            <div style={{ padding: 12, borderRadius: 12, border: "1px solid #e5e7eb", background: "white", color: "#6b7280" }}>
              <div style={{ fontSize: 14 }}>
                Need to edit your cart? <Link href="/cart">Go back to cart</Link>
              </div>
            </div>
